test_that("model c fits are accurate", {
  pred <- gestage_c(gagelmp = 250, birthwt = 2800)
  # filter(mc, gagelmp == 250, birthwt == 2800) %>% pull(fit)
  fit <- actual <- round(269.2573)
  expect_equal(pred, fit)

  pred <- gestage_c(gagelmp = 250, birthwt = 2800, round_pred = FALSE)
  fit <- actual <- 269.2573
  expect_true(abs(pred - fit) < 0.002)

  pred <- gestage_c(201:250, 2800, round_pred = FALSE)
  ...
  pred <- gestage_c(250, bwt, round_pred = FALSE)
...
test_that("model c doesn't accept garbage", {
  expect_error(gestage_c(240:245, 3000:3002),
    regexp = "must be the same")

  expect_error(gestage_c(150, 3000),
    "must be >= 161")

  expect_error(gestage_c(400, 3000),
    "must be <= 350")

  expect_error(gestage_c(200, 500),
    "must be >= 1000")

  expect_error(gestage_c(200, 6000),
    "must be <= 5000")
})
